...
$(function() {
    //we are checking to see if the user is signed in
    var currentUser = Parse.User.current();
    if (!currentUser) {   //if there is no current user (its null)
        window.location = 'signin.html';
    }
    $('nav-link-sign-out').click(function(){
        evt.preventDefault();
        Parse.User.LogOut();
        window.location = 'signin.html';
    });
    ...
    $('.form-new-task').submit(function(evt) {
        ...
        newTask.save().then(function() {  //promise for asynchronous.
            tasks.add(newTask);
            newTitleInput.val(''); //clearing the input
